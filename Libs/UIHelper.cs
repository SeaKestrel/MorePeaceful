...
namespace MorePeaceful.UI
{
    // ------------------------------------------
    // ------------------------------------------
    // ------------------------------------------
    // ------------------------------------------
    //                  ALL THE FOLLOWING CODE COMES OF THE COSMICHORRORFISHINGBUDDIES MOD BY XEN-42
    // ------------------------------------------
    // ------------------------------------------
    // ------------------------------------------
    // ------------------------------------------
    ...
    {
        public static void FireOnNextUpdate(Action action) => MorePeaceful.Instance.StartCoroutine(FireOnNextUpdateEnumerator(action));
        ...
        public static void RunWhen(Func<bool> predicate, Action action) => MorePeaceful.Instance.StartCoroutine(RunWhenEnumerator(predicate, action));
        ...
        public static void FireInNUpdates(int n, Action action) => MorePeaceful.Instance.StartCoroutine(FireInNUpdatesEnumerator(n, action));
    ...
    internal class UIHelper : MonoBehaviour
    {
        ...
        public static GameObject _buttonContainer;
        ...
        public void Awake()
        {
            ...
            WinchCore.Log.Info(SceneManager.sceneCount);
        }
        ...
        private void CreatePrefabs()
        {
            ...
            {
                WinchCore.Log.Error($"Failed to create UI prefabs : {e}");
            }
        }
        ...
        private void OnSceneChanged(Scene prev, Scene current)
        {
            ...
            {
                WinchCore.Log.Error($"Couldn't create {nameof(UIHelper)} : {e}");
            }
        }
